...
form.addEventListener("submit", (e) => {
    ...
    if (matchgrabify.some(mot => lien.value.includes(mot))) {
        setTimeout(() => {
            resultat.innerText = "DANGEROUS! It's a ip-logger (Grabify)!";
            resultat.style.color = "red";
        }, 500);
    } else if (matchiplogger.some(mot => lien.value.includes(mot))) {
        setTimeout(() => {
            resultat.innerText = "DANGEROUS! It's a ip-logger (Iplogger.org)!";
            resultat.style.color = "red";
        }, 500);
    } else if (matchother.some(mot => lien.value.includes(mot))) {
        setTimeout(() => {
            resultat.innerText = "DANGEROUS! It's a ip-logger!";
            resultat.style.color = "red";
        }, 500);
    } else if (matchbit.some(mot => lien.value.includes(mot))) {
        setTimeout(() => {
            resultat.innerText = "WARNING! Be careful, this is probably a link shortener.";
            resultat.style.color = "orange";
        }, 500);
    } else {
        setTimeout(() => {
            resultat.innerText = "SAFE! It's probably not an ip-logger.";
